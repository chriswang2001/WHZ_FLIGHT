...
float mRes[8] = {0.73f, 0.92f, 0.82f, 0.66f, 0.44f, 0.39f, 0.33f, 0.23f};

int16_t accelCount[3], gyroCount[3], magCount[3]; // Stores the 16-bit signed accelerometer, gyro, magnetometer sensor output
...
{
    ReadAccelRaw(accelCount); // Read the x/y/z adc values

    // Now we'll calculate the accleration value into actual g's
    ax = (float)accelCount[0] * aRes[Ascale] - accelBias[0]; // get actual g value, this depends on scale being set
    ay = (float)accelCount[1] * aRes[Ascale] - accelBias[1];
    az = (float)accelCount[2] * aRes[Ascale] - accelBias[2];

    ReadGyroRaw(gyroCount); // Read the x/y/z adc values

    // Calculate the gyro value into actual degrees per second
    gyrox = (float)gyroCount[0] * gRes[Gscale] - gyroBias[0]; // get actual gyro value, this depends on scale being set
    gyroy = (float)gyroCount[1] * gRes[Gscale] - gyroBias[1];
    gyroz = (float)gyroCount[2] * gRes[Gscale] - gyroBias[2];

    tempCount = ReadTempRaw();                         // Read the x/y/z adc values
    temperature = ((float)tempCount) / 340.f + 36.53f; // Temperature in degrees Centigrade
    ...
    printf("X-gyro rate: %.1f degrees/sec\n", gyrox);
    printf("Y-gyro rate: %.1f degrees/sec\n", gyroy);
    printf("Z-gyro rate: %.1f degrees/sec\n", gyroz);

    // Print temperature in degrees Centigrade
    printf("Temperature is %.2f degrees C\n", temperature);

    ReadMagRaw(magCount); // Read the x/y/z adc values
    ...
    mx = (float)magCount[0] * mRes[Mscale] - magbias[0]; // get actual magnetometer value, this depends on scale being set
    my = (float)magCount[1] * mRes[Mscale] - magbias[1];
    mz = (float)magCount[2] * mRes[Mscale] - magbias[2];
}
...
static void Sensor_WriteByte(uint8_t addr, uint8_t data)
{
    HAL_StatusTypeDef s = HAL_I2C_Master_Transmit(&hi2c1, addr << 1, &data, 1, I2Cx_TIMEOUT);
}
 ...
 */
static void Sensor_RegWriteByte(uint8_t addr, uint8_t reg, uint8_t data)
{
    HAL_StatusTypeDef s = HAL_I2C_Mem_Write(&hi2c1, addr << 1, reg, I2C_MEMADD_SIZE_8BIT, &data, 1, I2Cx_TIMEOUT);
}

/**
 * @brief read a byte to targer sensor's regsiter by I2C bus
 * @param addr address of sensor
 * @param reg address of target regsiter
 * @return uint8_t data read
 */
static uint8_t Sensor_ReadByte(uint8_t addr, uint8_t reg)
{
    uint8_t data; // data will store the register data
    HAL_StatusTypeDef s = HAL_I2C_Mem_Read(&hi2c1, addr << 1, reg, I2C_MEMADD_SIZE_8BIT, &data, 1, I2Cx_TIMEOUT);

    return data; // Return data read from slave register
}

/**
 * @brief write bytes to targer sensor's regsiter by I2C bus
 * @param addr address of sensor
 * @param reg address of target regsiter
 * @param length the length of data
 * @param data the pointer of data read
 */
static void Sensor_ReadBytes(uint8_t addr, uint8_t reg, uint8_t length, uint8_t *data)
{
    HAL_StatusTypeDef s = HAL_I2C_Mem_Read(&hi2c1, addr << 1, reg, I2C_MEMADD_SIZE_8BIT, data, length, I2Cx_TIMEOUT);
}
...
void ReadAccelRaw(int16_t *dest)
{
    ...
    Sensor_ReadBytes(MPU6050_ADDRESS, ACCEL_XOUT_H, 6, &rawData[0]); // Read the six raw data registers into data array
...
void ReadGyroRaw(int16_t *dest)
{
    ...
    Sensor_ReadBytes(MPU6050_ADDRESS, GYRO_XOUT_H, 6, &rawData[0]); // Read the six raw data registers sequentially into data array
    dest[0] = (int16_t)((rawData[0] << 8) | rawData[1]);            // Turn the MSB and LSB into a signed 16-bit value
    dest[1] = (int16_t)((rawData[2] << 8) | rawData[3]);
    dest[2] = (int16_t)((rawData[4] << 8) | rawData[5]);
}
...
{
    uint8_t rawData[6];                                                   // x/y/z gyro register data stored here
    Sensor_ReadBytes(HMC5883L_ADDRESS, HMC5883L_OUT_X_H, 6, &rawData[0]); // Read the six raw data registers sequentially into data array
    dest[0] = ((int16_t)rawData[0] << 8) | rawData[1];                    // Turn the MSB and LSB into a signed 16-bit value
...
//     Sensor_ReadBytes(MPU6050_ADDRESS, TEMP_OUT_H, 2, &rawData[0]); // Read the two raw data registers sequentially into data array
...
{
    uint8_t c = Sensor_ReadByte(MPU6050_ADDRESS, WHO_AM_I_MPU6050); // Read WHO_AM_I register for MPU-6050
...
void MPU_Calibrate(float *gyrobias, float *accbias)
{
    ...
    Sensor_RegWriteByte(MPU6050_ADDRESS, PWR_MGMT_1, 0x80); // Write a one to bit 7 reset bit; toggle reset device
    HAL_Delay(100);

    // get stable time source
    // Set clock source to be PLL with x-axis gyroscope reference, bits 2:0 = 001
    Sensor_RegWriteByte(MPU6050_ADDRESS, PWR_MGMT_1, 0x01);
    Sensor_RegWriteByte(MPU6050_ADDRESS, PWR_MGMT_2, 0x00);
    HAL_Delay(200);

    // Configure device for bias calculation
    Sensor_RegWriteByte(MPU6050_ADDRESS, INT_ENABLE, 0x00);   // Disable all interrupts
    Sensor_RegWriteByte(MPU6050_ADDRESS, FIFO_EN, 0x00);      // Disable FIFO
    Sensor_RegWriteByte(MPU6050_ADDRESS, PWR_MGMT_1, 0x00);   // Turn on internal clock source
    Sensor_RegWriteByte(MPU6050_ADDRESS, I2C_MST_CTRL, 0x00); // Disable I2C master
    Sensor_RegWriteByte(MPU6050_ADDRESS, USER_CTRL, 0x00);    // Disable FIFO and I2C master modes
    Sensor_RegWriteByte(MPU6050_ADDRESS, USER_CTRL, 0x0C);    // Reset FIFO and DMP
    HAL_Delay(15);

    // Configure MPU6050 gyro and accelerometer for bias calculation
    Sensor_RegWriteByte(MPU6050_ADDRESS, CONFIG, 0x01);       // Set low-pass filter to 188 Hz
    Sensor_RegWriteByte(MPU6050_ADDRESS, SMPLRT_DIV, 0x00);   // Set sample rate to 1 kHz
    Sensor_RegWriteByte(MPU6050_ADDRESS, GYRO_CONFIG, 0x00);  // Set gyro full-scale to 250 degrees per second, maximum sensitivity
    Sensor_RegWriteByte(MPU6050_ADDRESS, ACCEL_CONFIG, 0x00); // Set accelerometer full-scale to 2 g, maximum sensitivity

    uint16_t gyrosensitivity = 131;    // = 131 LSB/degrees/sec
    uint16_t accelsensitivity = 16384; // = 16384 LSB/g

    // Configure FIFO to capture accelerometer and gyro data for bias calculation
    Sensor_RegWriteByte(MPU6050_ADDRESS, USER_CTRL, 0x40); // Enable FIFO
    Sensor_RegWriteByte(MPU6050_ADDRESS, FIFO_EN, 0x78);   // Enable gyro and accelerometer sensors for FIFO  (max size 1024 bytes in MPU-6050)
    HAL_Delay(80);                                         // accumulate 80 samples in 80 milliseconds = 960 bytes

    // At end of sample accumulation, turn off FIFO sensor read
    Sensor_RegWriteByte(MPU6050_ADDRESS, FIFO_EN, 0x00);         // Disable gyro and accelerometer sensors for FIFO
    Sensor_ReadBytes(MPU6050_ADDRESS, FIFO_COUNTH, 2, &data[0]); // read FIFO sample count
    ...
    for (ii = 0; ii < packet_count; ii++)
    {
        ...
        Sensor_ReadBytes(MPU6050_ADDRESS, FIFO_R_W, 12, &data[0]);    // read data for averaging
    ...
    Sensor_RegWriteByte(MPU6050_ADDRESS, XG_OFFS_USRH, data[0]); // might not be supported in MPU6050
    Sensor_RegWriteByte(MPU6050_ADDRESS, XG_OFFS_USRL, data[1]);
    Sensor_RegWriteByte(MPU6050_ADDRESS, YG_OFFS_USRH, data[2]);
    Sensor_RegWriteByte(MPU6050_ADDRESS, YG_OFFS_USRL, data[3]);
    Sensor_RegWriteByte(MPU6050_ADDRESS, ZG_OFFS_USRH, data[4]);
    Sensor_RegWriteByte(MPU6050_ADDRESS, ZG_OFFS_USRL, data[5]);
    ...
    Sensor_ReadBytes(MPU6050_ADDRESS, XA_OFFSET_H, 2, &data[0]); // Read factory accelerometer trim values
    accel_bias_reg[0] = (int16_t)((int16_t)data[0] << 8) | data[1];
    Sensor_ReadBytes(MPU6050_ADDRESS, YA_OFFSET_H, 2, &data[0]);
    accel_bias_reg[1] = (int16_t)((int16_t)data[0] << 8) | data[1];
    Sensor_ReadBytes(MPU6050_ADDRESS, ZA_OFFSET_H, 2, &data[0]);
    ...
    Sensor_RegWriteByte(MPU6050_ADDRESS, XA_OFFSET_H, data[0]); // might not be supported in MPU6050
    Sensor_RegWriteByte(MPU6050_ADDRESS, XA_OFFSET_L_TC, data[1]);
    Sensor_RegWriteByte(MPU6050_ADDRESS, YA_OFFSET_H, data[2]);
    Sensor_RegWriteByte(MPU6050_ADDRESS, YA_OFFSET_L_TC, data[3]);
    Sensor_RegWriteByte(MPU6050_ADDRESS, ZA_OFFSET_H, data[4]);
    Sensor_RegWriteByte(MPU6050_ADDRESS, ZA_OFFSET_L_TC, data[5]);
...
bool MPU_SelfTest(void) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
    ...
    Sensor_RegWriteByte(MPU6050_ADDRESS, ACCEL_CONFIG, 0xF0);   // Enable self test on all three axes and set accelerometer range to +/- 8 g
    Sensor_RegWriteByte(MPU6050_ADDRESS, GYRO_CONFIG, 0xE0);    // Enable self test on all three axes and set gyro range to +/- 250 degrees/s
    HAL_Delay(250);                                             // Delay a while to let the device execute the self-test
    rawData[0] = Sensor_ReadByte(MPU6050_ADDRESS, SELF_TEST_X); // X-axis self-test results
    rawData[1] = Sensor_ReadByte(MPU6050_ADDRESS, SELF_TEST_Y); // Y-axis self-test results
    rawData[2] = Sensor_ReadByte(MPU6050_ADDRESS, SELF_TEST_Z); // Z-axis self-test results
    rawData[3] = Sensor_ReadByte(MPU6050_ADDRESS, SELF_TEST_A); // Mixed-axis self-test results
...
void MPU_Init(void)
{
    ...
    // get stable time source
    Sensor_RegWriteByte(MPU6050_ADDRESS, PWR_MGMT_1, 0x01); // Set clock source to be PLL with x-axis gyroscope reference, bits 2:0 = 001

    // Configure Gyro and Accelerometer
    // Disable FSYNC and set accelerometer and gyro bandwidth to 44 and 42 Hz, respectively;
    // DLPF_CFG = bits 2:0 = 010; this sets the sample rate at 1 kHz for both
    // Maximum delay time is 4.9 ms corresponding to just over 200 Hz sample rate
    Sensor_RegWriteByte(MPU6050_ADDRESS, CONFIG, 0x03);

    // Set sample rate = gyroscope output rate/(1 + SMPLRT_DIV)
    Sensor_RegWriteByte(MPU6050_ADDRESS, SMPLRT_DIV, 0x04); // Use a 200 Hz rate; the same rate set in CONFIG above

    // Set gyroscope full scale range
    // Range selects FS_SEL and AFS_SEL are 0 - 3, so 2-bit values are left-shifted into positions 4:3
    uint8_t c = Sensor_ReadByte(MPU6050_ADDRESS, GYRO_CONFIG);
    Sensor_RegWriteByte(MPU6050_ADDRESS, GYRO_CONFIG, c & ~0xE0);       // Clear self-test bits [7:5]
    Sensor_RegWriteByte(MPU6050_ADDRESS, GYRO_CONFIG, c & ~0x18);       // Clear AFS bits [4:3]
    Sensor_RegWriteByte(MPU6050_ADDRESS, GYRO_CONFIG, c | Gscale << 3); // Set full scale range for the gyro

    // Set accelerometer configuration
    c = Sensor_ReadByte(MPU6050_ADDRESS, ACCEL_CONFIG);
    Sensor_RegWriteByte(MPU6050_ADDRESS, ACCEL_CONFIG, c & ~0xE0);       // Clear self-test bits [7:5]
    Sensor_RegWriteByte(MPU6050_ADDRESS, ACCEL_CONFIG, c & ~0x18);       // Clear AFS bits [4:3]
    Sensor_RegWriteByte(MPU6050_ADDRESS, ACCEL_CONFIG, c | Ascale << 3); // Set full scale range for the accelerometer

    // Enable I2C_BYPASS_EN so additional chips
    // can join the I2C bus and all can be controlled by the Arduino as master
    Sensor_RegWriteByte(MPU6050_ADDRESS, INT_PIN_CFG, 0x02);
}
